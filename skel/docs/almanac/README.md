...
**Two categories have no destination here yet, and the omission is deliberate.** This
repository has no `docs/arch/`, no `docs/specs/`, and no issue tracker, so designed
intent, specifications and plans, and the status of in-flight work have nowhere to be
filed. Do not invent one, and do not let the almanac become the fallback: say the
destination is missing and stop. When one of those directories or trackers actually
exists, add its row.

Entries wrap at **88 columns**, matching `line-length` in `pyproject.toml`. Nothing
enforces the width on Markdown, so it is a convention you apply by hand rather than one
a formatter applies for you.
