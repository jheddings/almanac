...
Find the repository root and read the applicable agent instructions. Check for:

- `docs/almanac/README.md`;
- another live `almanac/README.md`, using the same exclusions as `record` and `audit` —
  ignore templates, examples, worktrees, dependencies, and nested checkouts;
...
Read the canonical contract at `${CLAUDE_PLUGIN_ROOT}/templates/almanac/README.md`. The
template's `<!-- almanac-template: N -->` comment and `<!-- almanac:local -->` block are
load-bearing; preserve both.
...
If the README already exists, preserve it. Report its template revision and propose only
clearly missing installation essentials. Do not replace its local block or silently
merge a newer template into it; repository-local conventions and template upgrades
require review.
...
Finally, repeat the inspection mentally against the resulting tree. A second invocation
