...
### `pyproject.toml` replaces `uv run --with`

CONTRIBUTING currently states the ephemeral-dependency posture as deliberate:
...
Adopt a `pyproject.toml` with a dev dependency group, so the invocation is plain
`uv run pytest`. The repo already depends on `uv`, the incantation gets shorter, and the
import path stops needing a `conftest.py` trick.

This trades away a stated design position, so it is recorded here as a decision rather
than absorbed as a side effect. The CONTRIBUTING passage describing the current posture
...
- Pre-commit hooks call `python3 -m tools ...` under `language: python`.
